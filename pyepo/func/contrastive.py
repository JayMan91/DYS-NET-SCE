...
class contrastiveMAP(optModule):
    ...
    def forward(self, pred_cost, true_cost, true_sol, true_obj, reduction="mean"):
        ...
        if np.random.uniform() <= solve_ratio:
            sol, obj = _solve_in_pass(cp, self.optmodel, self.processes, self.pool)
            if solve_ratio < 1:
                self.solpool = np.concatenate((self.solpool, sol))
                # remove duplicate
                self.solpool = np.unique(self.solpool, axis=0)     
            ...
            sol, obj = _cache_in_pass(cp, self.optmodel, self.solpool)        
        sol = torch.tensor(np.array(sol)).to(device)
        if self.optmodel.modelSense == EPO.MINIMIZE:
            loss = ((pred_cost )*(true_sol - sol)).sum(axis =1 )
        if self.optmodel.modelSense == EPO.MAXIMIZE:
            loss = -((pred_cost )*(true_sol - sol)).sum(axis =1 )
...
class contrastiveMAP_linear(optModule):
    """
    An autograd module for Maximum A Posterior contrastive estimation as
    surrogate loss functions, which is a efficient self-contrastive algorithm.
    ...
    def forward(self, pred_cost, true_cost, true_sol, true_obj, reduction="mean"):
        ...
        if np.random.uniform() <= solve_ratio:
            ...
            sol, obj = _cache_in_pass(cp, self.optmodel, self.solpool)
        sol = torch.FloatTensor(np.array(sol)).to(device)
        if self.optmodel.modelSense == EPO.MINIMIZE:
            loss = ((pred_cost - true_cost)*(true_sol - sol)).sum(axis =1 )
        if self.optmodel.modelSense == EPO.MAXIMIZE:
            loss = -((pred_cost  - true_cost)*(true_sol - sol)).sum(axis =1 )
...
# class contrastiveMAP_cminuspredc(optModule):
#     """
#     An autograd module for Maximum A Posterior contrastive estimation as
#     surrogate loss functions, which is a efficient self-contrastive algorithm.
...
#         solpool = torch.from_numpy(self.solpool.astype(np.float32)).to(device)
#         # get current obj
#         obj_cp = torch.einsum("bd,bd->b", pred_cost- true_cost, true_sol).unsqueeze(1)
#         # get obj for solpool
#         objpool_cp = torch.einsum("bd,nd->bn", pred_cost- true_cost, solpool)
#         # get loss
#         if self.optmodel.modelSense == EPO.MINIMIZE:
#             loss, _ = (obj_cp - objpool_cp).max(axis=1)
#         if self.optmodel.modelSense == EPO.MAXIMIZE:
