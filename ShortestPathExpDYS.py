    ...
    if str(grid_size) in map(str, model_config['hyperparameters'].keys()):
        ...
        full_config = {
            **common_params,
            'model_name': model_name,
            'grid_size': grid_size,
            'lr': 0.001,       # Default value, will be overridden if specified
...
parser.add_argument("--config", type=str, help="path to config file", default="configs/shortestpath_DYSconfig.json")
...
if 'DYS' in modelname:
    ...
    logger = CSVLogger(log_dir, name='dys{}_normalize{}_deg{}_noise{}_gridsize{}'.format(dflloss, 
                normalize, deg, noise_width, grid_size))
