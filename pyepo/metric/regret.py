...
def regret(predmodel, optmodel, dataloader):
    ...
    loss = np.zeros ( cp.shape[0] )
    for j in range(cp.shape[0]):
        loss[j] = calRegret(optmodel, cp[j], c[j].to("cpu").detach().numpy(),
                            z[j].item())
    
    predmodel.train()
    optsum = abs(z).detach().numpy()
    ## Both loss and z are vectors
    ## We will do element wise division and then take mean
    return (loss/(optsum + 1e-7)).mean()
...
def calRegret(optmodel, pred_cost, true_cost, true_obj):
    ...
    optmodel.setObj(pred_cost)
    obj = optmodel.PrescribedObj (true_cost)
    # loss
    if optmodel.modelSense == EPO.MINIMIZE:
        loss = obj - true_obj
        # print ("Obj", obj, true_obj)
