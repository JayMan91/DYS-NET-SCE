...
class vrpABModel(optGrbModel):
    ...
    def __init__(self, num_nodes, demands, capacity, num_vehicle):
        ...
        Args:
            ...
            num_vehicle (int): number of vehicle
        """
        self.num_nodes = num_nodes + 1  # adding 1 for depot
    ...
    def copy(self):
        ...
        """
        new_model = type(self)(self.num_nodes - 1, self.demands,
...
class vrpModel(vrpABModel):
    ...
    def _getModel(self):
        ...
        # varibles
        x = m.addVars( self.edges + [(i, 0) for i in self.nodes if i > 0 ] , name="x", vtype=GRB.BINARY)
        ...
        # constraints
        m.addConstr(x.sum(0, "*")  + x.sum("*", 0) <= 2 * self.num_vehicle) # depot degree
        m.addConstrs(x.sum(i, "*") + x[0, i] == 2 for i in self.nodes if i != 0)  # 2 degree
    ...
    def setObj(self, c):
        ...
        obj1 = gp.quicksum(c[k] * (self.x[i,j])
                          for k, (i,j) in enumerate(self.edges))

        obj2 = gp.quicksum(c[k] * (self.x[j, i])
                          for k, (i,j) in enumerate(self.edges) if i==0 )
    ...
    def solve(self):
        ...
        if self._model.Status == GRB.INFEASIBLE:
            ...
            self._model.write('iismodel.ilp')
        else:
            sol = np.zeros(len(self.edges), dtype=np.uint8)
            for k, (i,j) in enumerate(self.edges):
                # print ("solve", k, i, j)
...
class vrpModelNonBinary (vrpABModel):
    """
    This class is optimization model for capacitated vehicle routing probelm
    ...
    def _getModel(self):
        ...
        # x = m.addVars(  [(i, j) for i in self.nodes for j in self.nodes if i > 0 and j>0 and i < j] , name="x", vtype=GRB.BINARY)
        # z = m.addVars(  [(0, i) for i in self.nodes if i > 0 ] , name="z", ub = 2, vtype=GRB.INTEGER)

        x = m.addVars(self.edges, name="x", vtype=GRB.BINARY)
        for i in self.nodes:
            if i >0:
                k = (0,i)
                x[k].VType = GRB.INTEGER
                x[k].ub = 2
        for i, j in self.edges:
            x[j, i] = x[i, j]
        # sense
    ...
    def solve(self):
        ...
        if self._model.Status == GRB.INFEASIBLE:
            ...
            for k, (i,j) in enumerate(self.edges):
                ...
                if self.x[i,j].x > 1+1e-2 or self.x[j,i].x > 1+1e-2:
                    sol[k] = 2
                elif self.x[i,j].x > 1e-2 or self.x[j,i].x > 1e-2:
                    sol[k] = 1
...
class vrpModel2(vrpABModel):
    ...
    def solve(self):
        ...
        """
        self._model.update()
        self._model.setParam('TimeLimit', 30)
        self._model.optimize()
